...
const PatientPerfilPage = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const apiUrl = "http://localhost:3100/api/patient/getVisualisePacient";
        const config = {
          method: "get",
  ...
  const handleSave = async () => {
    try {
      ...
      const accessToken = await LocalStorageServices.GetData("accessToken");
      const apiUrl = "http://localhost:3100/api/patient/updateGetPatient";
      const config = {
        method: "put",
        url: apiUrl,
        headers: {
  ...
  return (
    <div className="patient-profile-container">
      ...
      {patientData && (
        <Card className="patient-profile-card" title="Detalles del Paciente">
          <Descriptions bordered>
            <Descriptions.Item label="Nombre">{patientData.name}</Descriptions.Item>
            <Descriptions.Item label="Apellido">{patientData.lastName}</Descriptions.Item>
            <Descriptions.Item label="DNI">{patientData.identityDoc}</Descriptions.Item>
            <Descriptions.Item label="Género">{patientData.gender}</Descriptions.Item>
            <Descriptions.Item label="Teléfono">{patientData.phone}</Descriptions.Item>
...
export default PatientPerfilPage;
